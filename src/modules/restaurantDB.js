...
module.exports = class RestaurantDB {
    ...
    getRestaurantsByCuisine(page, perPage, cuisine) {
        let cuisineFind = cuisine ? { cuisine } : {}
        if (+page && +perPage) {
            return this.Restaurant.find(cuisineFind)
                .sort({ restaurant_id: +1 })
                .skip(page * +perPage)
                .limit(+perPage)
                .exec()
        }
        return Promise.reject(new Error('cuisines must be present'))
    }
